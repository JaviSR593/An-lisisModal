...
import flet as ft
import time
import threading # Usamos hilos para no congelar la pantalla
...
def main(page: ft.Page):
    ...
    # ==========================================
    #   LÓGICA DE LA APP (Definida primero)
    # ==========================================
    def iniciar_interfaz_principal():
        page.clean() # Borramos el splash
    ...
    creditos = ft.Text("Por Dayana Guanotuña", italic=True, color="cyan")

    col_splash = ft.Column([
        ft.Container(height=100),
        icono, ft.Container(height=10),
        titulo, ft.Container(height=40),
        spinner, ft.Container(height=40),
        creditos
    ], horizontal_alignment="center")

    page.add(col_splash)
    
    # Usamos un temporizador para cambiar de pantalla sin congelar la app
    # Esto evita la pantalla negra inicial
    def timer_carga():
        time.sleep(3)
        # Importante: Las actualizaciones de UI desde otro hilo deben ir en page.run_task o similar
        # pero en Flet simple, llamamos a la función de construcción
        iniciar_interfaz_principal()

    # Ejecutamos el temporizador en segundo plano
    t = threading.Thread(target=timer_carga)
    t.start()
